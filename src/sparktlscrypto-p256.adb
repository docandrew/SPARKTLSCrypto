...
package body SPARKTLSCrypto.P256 with
   SPARK_Mode => On
is
   ...
   function Add_F256 (A, B : P256_FE) return P256_FE is
     (SPARKTLSCrypto.Fiat_P256.Add (A, B));

   function Sub_F256 (A, B : P256_FE) return P256_FE is
     (SPARKTLSCrypto.Fiat_P256.Sub (A, B));

   function Mul_F256 (A, B : P256_FE) return P256_FE is
     (SPARKTLSCrypto.Fiat_P256.Mul (A, B));

   function Square_F256 (A : P256_FE) return P256_FE is
     (SPARKTLSCrypto.Fiat_P256.Sqr (A));

   function FE_Is_Zero (A : P256_FE) return Boolean is
      T : Unsigned_64;
   begin
      SPARKTLSCrypto.Fiat_P256.Nonzero (T, A);
      return T = 0;
   end FE_Is_Zero;
   ...
   begin
      ...
      Non_Monty := SPARKTLSCrypto.Fiat_P256.From_Bytes (LE);
      Dst := SPARKTLSCrypto.Fiat_P256.To_Montgomery (Non_Monty);
   end Bytes_To_FE;
   ...
   is
      ...
      LE : Byte_Seq (0 .. 31);
   begin
      Non_Monty := SPARKTLSCrypto.Fiat_P256.From_Montgomery (Src);
      SPARKTLSCrypto.Fiat_P256.To_Bytes (LE, Non_Monty);
      --  Reverse little-endian → big-endian
      for I in I32 range 0 .. 31 loop
         pragma Loop_Invariant
           (for all K in I32 range 0 .. I - 1 =>
              Dst (Dst'First + K)'Initialized);
   ...
   begin
      ...
      for J in reverse Src'Range loop
         pragma Loop_Invariant (DI in 0 .. 8 and Acc_Len in 0 .. 29);
         B := U32 (Src (J));
         if Acc_Len < 22 then
   ...
   begin
      for J in reverse Dst'Range loop
         pragma Loop_Invariant (SI in 0 .. 9 and Acc_Len in 0 .. 29);
         if Acc_Len < 8 then
            if SI <= 8 then
               W := Src (SI);
               SI := SI + 1;
