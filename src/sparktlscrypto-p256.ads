...
package SPARKTLSCrypto.P256 with
   SPARK_Mode => On
is
   ---------------------------------------------------------------
   --  Field element type (4×64-bit Montgomery, from Fiat Crypto)
   ---------------------------------------------------------------
   subtype P256_FE is SPARKTLSCrypto.Fiat_P256.FE;

   --  Zero and one in Montgomery domain
   FE_Zero : constant P256_FE := SPARKTLSCrypto.Fiat_P256.FE_Zero;
   FE_One  : constant P256_FE := SPARKTLSCrypto.Fiat_P256.FE_One;
   ...
   function Add_F256 (A, B : P256_FE) return P256_FE with Inline;

   function Sub_F256 (A, B : P256_FE) return P256_FE with Inline;

   function Mul_F256 (A, B : P256_FE) return P256_FE with Inline;

   function Square_F256 (A : P256_FE) return P256_FE with Inline;
   ...
   procedure Mul9
     (D :    out P256_Wide;
      A : in     P256_Limbs;
      B : in     P256_Limbs)
   with Relaxed_Initialization => D, Post => D'Initialized;
