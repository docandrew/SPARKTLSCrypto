...
is
   ...
   procedure P256_Muladd
     (A    : in out Byte_Seq;
      ...
      Ylen : in     N32;
      OK   :    out U32)
   with Pre => A'Length = 65 and then A'First = 0
               and then X'First = 0 and then X'Length <= 32 and then Xlen <= X'Length
               and then Y'First = 0 and then Y'Length <= 32 and then Ylen <= Y'Length;
